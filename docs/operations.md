...
`packet-svc-auth-forge` is centered on backend services work in Kotlin. The technical target is: Design a Kotlin verification harness for auth systems, covering incremental indexing, append-only fixtures, and failure-oriented tests.
...
The score is deterministic. The current threshold is 166. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 5.
